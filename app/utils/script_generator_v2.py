...
class ScriptProcessor:
    ...
    def _get_default_prompt(self) -> str:
        return f"""
**背景：**
现在有一个关于{self.video_theme}【video】的影视作品

**角色：**
你是一名影视解说专家，拥有丰富的影视解说经验

**目标：**
1. 解说【video】里的每个章节的主要事件片段

**要求：**
1. 事件解说时的描述尽量详细：什么人做了什么事情产生了什么结果、呈现了什么样的画面；什么时间什么地点发生了什么事情、有什么样的画面
2. 一个连续的故事片段可以拆成多个解说片段
3. 每个解说片段的字数尽量多一些，至少3到5句话
4. 不是【video】的剧情不要解说
5. 没有的剧情不要凭空捏造，要实事求是
6. 如果解说内容涉及到专业知识或历史知识，可以3到5句话说说
7. 文案字数在 1000字左右，严格要求字数，最低不得少于 500字。
8. 结果输出为JSON数组格式，每个索引对象包含字段：
    - "narration"：具体的解说内容
9. 请以严格的 JSON 格式返回数据，不要包含任何注释、标记或其他字符。数据应符合 JSON 语法，可以被 json.loads() 函数直接解 析， 不要添加 ```json 或其他标记。
...
"""
    ...
    def _get_default_prompt2(self) -> str:
        return f"""
**背景：**
现在有一个关于{self.video_theme}【video】的影视作品
...
"""
    ...
    def _get_default_prompt_src(self) -> str:
        return f"""
